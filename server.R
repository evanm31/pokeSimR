  ...
  observeEvent(input$pokemon, handlerExpr = {
    ...
    observeEvent(input$type, {
      ...
      if (input$type == "max") {
        ...
        sim <- which.min(mat.sim[index,-(index)])
      }
      ...
      output$pokeInfo <- renderPrint({
        ...
        if (input$type == "min"){
          cat("The least similar Pokémon to ", input$pokemon, " is:")
        }
      })
    ...
    observeEvent(input$searchInput, handlerExpr = {
      ...
      output$pokeInfo <- renderPrint({
        if (input$type == "max") {
          ...
          cat("The least similar Pokémon to ", input$textIn, " is:")
        }
      })
   ...
   observeEvent(input$newPoke, handlerExpr = {
     ...
     output$pokeInfo <- renderPrint({
       if (input$type == "max") {
         ...
         cat("The least similar Pokémon to ", pok, " is:")
       }
     })
